...
DIST = ROOT / 'dist'
app = FastAPI(title='GitPix API', version='2.0.0', docs_url=None, redoc_url=None)
_cache = {}
...
def security_headers(response):
    ...
    response.headers['Content-Security-Policy'] = "default-src 'self'; img-src 'self' https: data:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; script-src 'self' https://fonts.googleapis.com; connect-src 'self' https:; frame-ancestors 'none'"
    return response
...
@app.get('/api/health')
async def health():
    return {'status': 'ok', 'service': 'gitpix'}
...
async def fetch_github_profile(username: str):
    ...
    if cached and monotonic() - cached['at'] < 300:
        return cached['data']
    headers = {'Accept': 'application/vnd.github+json', 'User-Agent': 'GitPix/2.0'}
...
async def github_oauth_start(request: Request):
    if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
        raise HTTPException(503, 'GitHub publishing is not configured yet.')
...
async def github_oauth_callback(request: Request, code: str = '', state: str = ''):
    ...
    response.delete_cookie('gitpix_oauth_state')
    response.set_cookie('gitpix_github_token', token, max_age=60 * 60 * 24 * 30, httponly=True, secure=request.url.scheme == 'https', samesite='lax')
    return response
...
async def github_publish(payload: PublishRequest, request: Request):
    rate_limit(request, 'publish', 10)
    token = request.cookies.get('gitpix_github_token')
    if not token:
        raise HTTPException(401, 'Connect GitHub before publishing.')
    headers = {'Accept': 'application/vnd.github+json', 'Authorization': f'Bearer {token}', 'User-Agent': 'GitPix/2.0'}
    async with httpx.AsyncClient(timeout=15) as client:
        me = await client.get('https://api.github.com/user', headers=headers)
        if not me.is_success:
            raise HTTPException(401, 'Your GitHub connection has expired. Reconnect.')
        user = me.json()
        username = user.get('login')
        repo_url = f'https://api.github.com/repos/{username}/{username}'
...
def clean_ai_markdown(text: str) -> str:
    text = re.sub(r'^```(?:markdown|md)?\s*', '', text.strip(), flags=re.I)
    ...
    try:
        async with httpx.AsyncClient(timeout=45) as client:
            if AI_PROVIDER == 'gemini':
                ...
                res = await client.post(url, json={'systemInstruction': {'parts': [{'text': system}]}, 'contents': [{'role': 'user', 'parts': [{'text': prompt}]}], 'generationConfig': {'temperature': 0.7, 'maxOutputTokens': 6000}})
                if not res.is_success:
        ...
        raise HTTPException(504, 'AI generation timed out. Try again.')
    except HTTPException:
        raise
    except Exception:
        raise HTTPException(502, 'AI generation failed.')
    text = clean_ai_markdown(text)
    if not text:
        raise HTTPException(502, 'The AI provider returned an empty README.')
    return {'markdown': text, 'provider': AI_PROVIDER, 'model': AI_MODEL}
