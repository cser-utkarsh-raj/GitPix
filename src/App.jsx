...
import { Copy, Download, Github, Search, RotateCcw, Check, ChevronRight, Eye, SlidersHorizontal, Sparkles, ExternalLink, Upload, LogOut, Wand2, ArrowUpRight } from 'lucide-react';
...
async function safeJson(response) {
  ...
  catch { return { ok: response.ok, data: { detail: text.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim() || 'The API returned an invalid response.' } }; }
}
...
export default function App() {
  ...
  useEffect(() => { fetch('/api/github/connection').then(safeJson).then(({ data }) => { setConnected(!!data.connected); setConnectedUser(data.username || ''); }).catch(() => {}); }, []);
  ...
  async function importGithub() {
    ...
    try {
      const response = await fetch(`/api/github/user/${encodeURIComponent(value)}`);
      const { ok, data } = await safeJson(response);
      if (!ok) throw new Error(data.detail || data.error || 'Could not load that GitHub profile.');
      ...
      setUsername(data.profile.username); setAiMarkdown(''); setGenerationMode('template');
  ...
  async function generateWithAI() {
    ...
    try {
      const response = await fetch('/api/ai/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ profile, template, instructions: `Use the ${selectedTemplate.name} style. Keep it distinctive, factual and polished. Do not invent projects, roles, employers, metrics or technologies.` }) });
      const { ok, data } = await safeJson(response);
      if (!ok) throw new Error(data.detail || 'AI generation failed.');
  ...
  function applyTemplate(id) { setTemplate(id); setAiMarkdown(''); setGenerationMode('template'); }
  function reset() { setProfile(EMPTY_PROFILE); setUsername(''); setTemplate('ronin'); setAiMarkdown(''); setGenerationMode('template'); setQueryError(''); setAiError(''); localStorage.removeItem('gitpix_profile'); localStorage.removeItem('gitpix_template'); }
  function connectGithub() { window.location.href = '/api/github/oauth/start'; }
  async function publishGithub() {
    setPublishState('publishing');
    try { const response = await fetch('/api/github/publish', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ markdown }) }); const { ok, data } = await safeJson(response); if (!ok) throw new Error(data.detail || 'Publish failed.'); setPublishState(data.url || 'published'); }
  ...
  return <div className={`gitpix-shell theme-${template}`}>
    ...
    <main id="top">
      <section className="hero"><div className="hero-copy"><div className="eyebrow"><span className="pulse-dot"/> GITHUB PROFILE README DESIGNER</div><h1>Turn your profile<br/><em>into a signature.</em></h1><p>Bring in your public GitHub data, choose a visual system, let AI write the Markdown or use a deterministic template, then publish it back to your profile repository.</p><div className="hero-cta"><a href="#designer" className="primary-btn">Start designing <ChevronRight size={17}/></a><a href="#templates" className="text-btn">Explore the systems <ArrowUpRight size={14}/></a></div><div className="hero-meta"><span>6 visual systems</span><i/> <span>GitHub import</span><i/> <span>AI + templates</span><i/> <span>Optional publish</span></div></div><div className="hero-art" aria-hidden="true"><div className="hero-ambient ambient-a"/><div className="hero-ambient ambient-b"/><div className="hero-frame"><div className="frame-top"><span>GITPIX / DESIGNER</span><span>01 — README</span></div><div className="frame-body"><div className="hero-avatar"><span>GP</span></div><div className="hero-lines"><b/><b/><b/><b/><b/></div><div className="hero-chip">MARKDOWN<br/><strong>READY</strong></div></div><div className="frame-bottom"><span>PROFILE → STORY</span><span>BUILD / EXPORT</span></div></div><div className="hero-orbit orbit-a"/><div className="hero-orbit orbit-b"/><div className="hero-label">GITHUB / README / 02</div></div></section>
      ...
      <section id="designer" className="section"><div className="section-head"><div><span className="section-kicker">02 / THE DESIGNER</span><h2>GitHub in. README out.</h2></div><p>Import the public profile once. GitPix turns repositories, languages, profile facts and stats into editable content.</p></div><div className="mobile-tabs"><button className={mobilePanel === 'editor' ? 'active' : ''} onClick={() => setMobilePanel('editor')}><SlidersHorizontal size={15}/> Edit</button><button className={mobilePanel === 'preview' ? 'active' : ''} onClick={() => setMobilePanel('preview')}><Eye size={15}/> Preview</button></div><div className="workbench">
        <aside className={`panel editor-panel ${mobilePanel === 'preview' ? 'mobile-hidden' : ''}`}><div className="panel-title"><div><span>1 / GITHUB PROFILE</span><small>Public data · no token required for import</small></div><Github size={18}/></div><div className="import-row"><div className="field with-icon"><Search size={15}/><input value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key === 'Enter' && importGithub()} placeholder="github username" aria-label="GitHub username"/></div><button className="square-btn" onClick={importGithub} disabled={queryLoading}>{queryLoading ? '…' : <ChevronRight size={18}/>}</button></div>{queryError && <div className="error-box">{queryError}</div>}
    ...
    <footer className="footer"><div className="footer-brand-block"><div className="brand footer-brand"><img src="/gitpix-logo.svg" alt="GitPix" /></div><p>Design your GitHub story.<br/>Built for people who care about the details.</p></div><div className="dot-footer"><span>PRESENTED BY</span><a href="https://github.com/cser-utkarsh-raj" target="_blank" rel="noreferrer" aria-label=".dot"><strong><b>.</b>dot</strong></a></div><div className="footer-links"><a href="https://github.com/cser-utkarsh-raj/GitPix" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={12}/></a><a href="#templates">Templates</a><a href="#designer">Designer</a></div></footer>
  </div>;
}
